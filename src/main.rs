        ...
        for y in 1..(h + 3) {
            for x in 1..(w + 3) {
                if (!m[y][x]) &
                        ...
                        | m[y][x - 1]
                        | m[y][x + 1]) {
                    let mut m2: Matrix = m.iter().map(
                            |row| row.to_vec()).collect();
                    m2[y][x] = true;
                    deflate(&mut m2);
                    let m3 = canonical(&m2);
...
fn main() {
    // FXYPTWNZLUVI
    let pentominos = next_generation(
            &next_generation(
            &next_generation(
            &next_generation(
...
fn get_compiter() -> Vec<(u64, u64)> {
    ...
    for y in 0..6 {
        for x in 0..10 {
            ...
            if x > 0 {
                around |= two.pow((y * 10 + x - 1) as u32);
            }
            if y > 0 {
                around |= two.pow(((y - 1) * 10 + x) as u32);
            }
        ...
        for candidate in candidates {
            if 0 == (candidate & current) {
                ...
                if holes_five(compiter, bitcounter, next) {
                    ...
                    find_solutions(compiter, bitcounter,
                            others.to_vec(), solution, next);
                    solution.pop();
                }
            }
        }
...
fn holes_of(compiter: &Vec<(u64, u64)>, board: u64) -> Vec<u64> {
    ...
    for (bit, around) in compiter {
        if 0 == (board & bit) {
            ...
            existing = falsy;
            truthy.push(*bit);
            let region = truthy.iter().copied().reduce(|a, b| a | b).unwrap();
...
mod tests {
    ...
    fn board_from(s1: &str, s2: &str, s3: &str, s4: &str,
            s5: &str, s6: &str) -> u64 {
        let two: u64 = 2;
        let mut n: u64 = 0;
